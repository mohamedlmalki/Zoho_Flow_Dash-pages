...
// --- API Routes ---
app.get('/api/flow-accounts', async (c) => c.json(await storage.getFlowAccounts()));
...
app.get('/api/templates', async (c) => c.json(await storage.getEmailTemplates()));
app.post('/api/templates', async (c) => {
    const data = await c.req.json();
    const parsed = insertEmailTemplateSchema.parse(data);
    return c.json(await storage.createEmailTemplate(parsed));
});
app.get('/api/campaigns', async (c) => c.json(await storage.getEmailCampaigns()));
app.get('/api/campaigns/:id', async (c) => c.json(await storage.getEmailCampaign(c.req.param('id'))));
...
app.post('/api/campaigns/:id/start', async (c) => c.json(await storage.updateEmailCampaign(c.req.param('id'), { status: 'running' })));
app.post('/api/campaigns/:id/pause', async (c) => c.json(await storage.updateEmailCampaign(c.req.param('id'), { status: 'paused' })));
app.post('/api/campaigns/:id/stop', async (c) => c.json(await storage.updateEmailCampaign(c.req.param('id'), { status: 'stopped' })));
app.get('/api/campaigns/:id/results', async (c) => c.json(await storage.getEmailResults(c.req.param('id'))));
app.delete('/api/campaigns/:id/results', async (c) => c.json(await storage.clearEmailResults(c.req.param('id'))));
...
// ... (The scheduled function remains the same) ...
